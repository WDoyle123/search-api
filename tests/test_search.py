...
def test_get_hotels_in_range_no_hotels_in_range(client, seed_events, db_session):
    ...
    far_hotel = models.Hotels(
        hotelID=999, latitude=40.7128, longitude=-74.0060)
...
def test_get_hotels_in_range_with_hotels_in_range(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&modes=walking,driving")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 3
    for hotel in data:
...
def test_get_hotels_in_range_invalid_modes(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&modes=invalid_mode")
    assert response.status_code == 400
...
def test_get_hotels_in_range_mixed_valid_invalid_modes(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&modes=walking,invalid_mode")
    assert response.status_code == 400
    assert "Invalid transport mode" in response.json()["detail"]
...
def test_get_hotels_in_range_all_transport_modes(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&modes=walking,driving,public_transport")
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
    for hotel in data:
...
def test_get_hotels_in_range_single_mode(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&modes=public_transport")
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
    for hotel in data:
...
def test_get_hotels_in_range_sort_by_distance(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&sort_by=distance")
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
    for i in range(len(data) - 1):
        assert data[i]["distance_km"] <= data[i + 1]["distance_km"]


def test_get_hotels_in_range_sort_by_estimated_time(client, seed_events, seed_hotels):
    response = client.get(
        "v1/search/?event_id=1&radius_km=10&sort_by=travel_time")
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
    for i in range(len(data) - 1):
        min_time_current = min(data[i]["estimated_travel"].values())
        min_time_next = min(data[i + 1]["estimated_travel"].values())
        assert min_time_current <= min_time_next
